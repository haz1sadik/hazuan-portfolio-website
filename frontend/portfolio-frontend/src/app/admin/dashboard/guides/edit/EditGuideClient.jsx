    ...
    useEffect(() => {
        ...
        const loadGuide = async () => {
            ...
            try {
                ...
                setGuideId(guideRes.data?.id || id);
                setTitle(guideRes.data?.title || "");
                setCategory(guideRes.data?.category || "");
                setDifficulty(guideRes.data?.difficulty || "");
                setContent(guideRes.data?.content || "");
                setThumbnailUrl(guideRes.data?.thumbnail_url || "");
            } catch (err) {
                if (!isMounted) return;
                ...
                {isLoading ? (
                    ...
                    <>
                        <div className="flex flex-wrap gap-6">
                            <TextInputField label="Title" name="title" placeholder="Enter guide title" value={title} onChange={(event) => setTitle(event.target.value)} required />
                            <TextInputField label="Category" name="category" placeholder="e.g. Proxmox" value={category} onChange={(event) => setCategory(event.target.value)} />
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <RichTextEditor value={content} onChange={setContent} onImageUpload={handleImageUpload} placeholder="Write your guide content here..." />
                        </div>
...
export default EditGuideClient;