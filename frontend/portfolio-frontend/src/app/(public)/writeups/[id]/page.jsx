...
export async function generateStaticParams() {
    ...
    if (!Array.isArray(writeups)) return [];
    return writeups.map((writeup) => ({ id: writeup.id }));
}

const WriteupPostPage = async ({ params }) => {
    const { id } = await params;
    const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api";
    const response = await fetch(`${baseUrl}/ctf-writeups/${id}`);
    ...
    const showUpdated = shouldShowUpdated(writeup.createdAt, writeup.updatedAt);
                ...
                <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-wrap gap-3 text-xs font-medium text-white/70">
                            <span>Created {formatDate(writeup.createdAt)}</span>
                            {showUpdated ? (
                    ...
                    <article className="rounded-3xl border border-[var(--layout-post-border)] bg-[var(--layout-post-surface)] px-5 py-6 text-[var(--layout-post-text)] shadow-[0_18px_45px_rgba(15,23,42,0.15)] sm:px-8 sm:py-8">
                        <div
                            ...
                            dangerouslySetInnerHTML={{ __html: buildContent(writeup.content) }}
                        />
                    </article>
                </div>
            </div>
...
export default WriteupPostPage;
