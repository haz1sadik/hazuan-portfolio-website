...
const GuidesClient = ({ initialGuides }) => {
    ...
    const categories = useMemo(() => {
        return Array.from(
            new Set(initialGuides.map((guide) => guide.category).filter(Boolean))
        );
    }, [initialGuides]);

    const difficulties = useMemo(() => {
        return Array.from(
            new Set(initialGuides.map((guide) => guide.difficulty).filter(Boolean))
        );
    }, [initialGuides]);
    ...
    }, [initialGuides, searchTerm, selectedCategory, selectedDifficulty]);
        ...
        <div className="border border-white/20 bg-[var(--layout-nav-bg)]/35 py-4 shadow-[0_8px_30px_rgba(255,255,255,0.25)] backdrop-blur-sm rounded-3xl px-5 sm:pb-16 pb-5 sm:pt-10 pt-7 mb-4 text-white">
            <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 sm:gap-8">
                <div className="flex flex-col gap-4">
                    <h1 className="sm:text-3xl text-2xl font-semibold tracking-tight">Guides</h1>
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                {difficulties.length === 0 ? (
                                    <span className="sm:text-sm text-xs text-neutral-400">No difficulty data</span>
                                ) : (
                                    <div className="flex flex-wrap gap-2">
                                        <button
                                            onClick={() => setSelectedDifficulty("All")}
                                            className={`rounded-full sm:px-4 px-3 sm:py-1.5 py-1 text-xs font-semibold uppercase tracking-wide transition hover:cursor-pointer ${selectedDifficulty === "All"
                                        ...
                                        {difficulties.map((difficulty) => (
                                            <button
                                                ...
                                                className={`rounded-full sm:px-4 px-3 sm:py-1.5 py-1 text-xs font-semibold uppercase tracking-wide transition hover:cursor-pointer ${selectedDifficulty === difficulty
                                                    ? "bg-neutral-900 text-white"
                                                    : "bg-white text-neutral-600 hover:bg-neutral-100"
                                                    }`}
                                            >
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                {categories.length === 0 ? (
                                    <span className="sm:text-sm text-xs text-neutral-400">No categories yet</span>
                                ) : (
                                    <div className="flex flex-wrap gap-2">
                                        <button
                                            onClick={() => setSelectedCategory("All")}
                                            className={`rounded-full sm:px-4 px-3 sm:py-1.5 py-1 text-xs font-semibold uppercase tracking-wide transition hover:cursor-pointer ${selectedCategory === "All"
                                                ? "bg-neutral-900 text-white"
                                                : "bg-white text-neutral-600 hover:bg-neutral-100"
                                                }`}
                                        >
                                        ...
                                        {categories.map((category) => (
                                            <button
                                                ...
                                                className={`rounded-full sm:px-4 px-3 sm:py-1.5 py-1 text-xs font-semibold uppercase tracking-wide transition hover:cursor-pointer ${selectedCategory === category
                                                    ? "bg-neutral-900 text-white"
                                                    : "bg-white text-neutral-600 hover:bg-neutral-100"
                                                    }`}
                                            >
                ...
                {filteredGuides.length === 0 ? (
                    <div className="text-sm text-neutral-500">No guides found.</div>
                ) : (
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        {filteredGuides.map((guide) => {
                            const thumbnail = guide.thumbnail_url || DEFAULT_THUMBNAIL;
                            return (
                                <article
                                    key={guide.id}
